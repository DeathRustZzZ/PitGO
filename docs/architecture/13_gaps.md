...
| `pub mod tests` | `infrastructure/lib.rs` | без `#[cfg(test)]`, в отличие от `application` и `backend` |
...
- **backend** — маршруты создания и чтения есть; контракт ошибок покрыт
  9 тестами, но сквозных тестов маршрутов нет.
