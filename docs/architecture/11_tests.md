...
Все 100 тестов, распределённые по четырём уровням, с указанием тестового
двойника.
...
flowchart LR
  ...
  subgraph L2["Уровень 2 — Приложение (20 тестов)"]
    A1["tests/customer.rs<br/>5 тестов"]
    A2["tests/vehicle.rs<br/>5 тестов"]
    A3["tests/ownership.rs<br/>10 тестов"]
  ...
  subgraph L4["Уровень 4 — HTTP-граница (17 тестов)"]
    B1["src/tests/error.rs<br/>10 unit-тестов маппинга ошибок"]
    B2["tests/api.rs<br/>7 интеграционных HTTP-сценариев"]
    WIRE["Router + oneshot<br/>контракт как у клиента"]
  end
...
| `backend` | `src/tests/` (unit) и `tests/api.rs` (integration), `#[cfg(test)] mod tests` в `lib.rs` | unit-тесты закрыты гейтом; integration-тесты используют только публичный Router |
...
**Побочный эффект выноса тестов из модуля.** Unit-тесты `backend` живут в
...
`backend/tests/api.rs` создаёт новый `AppState::in_memory()` для каждого теста
и отправляет `Request` напрямую в `Router` через `tower::ServiceExt::oneshot`.
Так закреплены health-check, customer create/get/404/duplicate, инвариант
одного открытого ownership, вложенный GET и 404 для ownership другого
автомобиля. TCP-сервер при этом не запускается: проверяется тот же HTTP-контракт
быстрее и без занятого порта.

Ошибки JSON-extractor тоже зафиксированы характеризационным тестом: сейчас они
возвращают стандартное текстовое тело axum, а не `{error, message}`. Унификация
этого тела остаётся отдельной задачей Фазы 2.
